...
class MPCController:
    ...
    def _construct_mpc(self):
        ...
        # 末端位置目标
        obj += 100 * ca.mtimes([(X[:, self.N] - P[self.n_states:]).T, self.Q, X[:, self.N] - P[self.n_states:]])
...
env = DoubleGyreEnvironment(render_mode='human', _init_t=None, is_fixed_start_and_target=False)
env.reset()
